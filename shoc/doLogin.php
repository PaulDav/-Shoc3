...
try {
	...
	$sql = "SELECT DISTINCT usrRecnum, usrName, usrSalt, usrPassword FROM tbl_user LEFT JOIN tbl_user_email ON emlUser=usrRecnum WHERE emlEmail='".$handle."' ";
	...
	if ($result->num_rows>0) {
		...
		if (!($rstRow['usrPassword'] == hash('sha256',$rstRow['usrSalt'].$passwd))){
			throw new Exception("Failed to login");
		}
