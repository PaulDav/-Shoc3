	...
	switch ($Mode){
		case "view":
			...
			if (is_object($objAccount)){
				$PanelB .= "<tr><th>Level</th><td>".$objAccount->UserLevel->Label."</td></tr>";				
			}
			...
			if (!$System->User->PictureOf == ""){
				$PanelB .= '<img src="image.php?Id='.$System->User->PictureOf.'" alt="picture uploaded by the user"  /><br/>';
			}
			$PanelB .= "</div>";
			...
			foreach ($System->User->Emails as $Email){
				$Seq = $Seq + 1;
				$PanelB .= "<tr><td>$Email</td><td/><td><a href='account.php?mode=edit&selection=email&seq=$Seq'>&bull; edit</a>";
				if ($Seq > 1){
					$PanelB .= " <a href='account.php?mode=delete&selection=email&seq=$Seq'>&bull; delete</a>";
				}
		...
		switch ($Selection){
			case "email":
				...
				$Page->Title = "$Mode email";
				$PanelB .= "<h1>".$Page->Title."</h1>";

				$Email = $System->User->Emails[$Seq - 1];
				...
				$PanelB .= "<input type='hidden' name='Mode' value='$Mode'/>";
				$PanelB .= "<input type='hidden' name='Selection' value='$Selection'/>";
				$PanelB .= "<input type='hidden' name='Seq' value='$Seq'/>";

				$PanelB .= '<table class="bluebox">';

				$PanelB .= '<tr>';
				$PanelB .= '<th>Your Email address</th>';
				$PanelB .= "<td>$Email</td>";
				$PanelB .= '</tr>';
				...
				$PanelB .= "</table>";
				$PanelB .= '<input type="submit" value="Confirm Delete Email from Account">';
...
function funAccountEmail(){
	...
	$Content .= "<input type='hidden' name='Seq' value='$Seq'/>";
